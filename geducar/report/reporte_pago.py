...
def num_a_letras(num, completo=True):
    ...
    if decimal == 0:
        letras = '%s quetzales' % num_en_letras
    else:
        letras = '%s quetzales con %s/100' % (num_en_letras, decimal)
...
class ReportePago(models.AbstractModel):
    _name = 'report.geducar.reporte_pago'
